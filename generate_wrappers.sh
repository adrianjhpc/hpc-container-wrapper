...
if [[ "$CW_MODE" == "wrapcont" ]];then
    ...
    _RUN_CMD="$CW_SINGULARITY_EXECUTABLE_PATH --silent exec --no-home \$DIR/../\$CONTAINER_IMAGE"
    _SHELL_CMD="$CW_SINGULARITY_EXECUTABLE_PATH --silent shell --no-home \$DIR/../\$CONTAINER_IMAGE"
else
    _CONTAINER_EXEC="$CW_SINGULARITY_EXECUTABLE_PATH --silent exec --no-home -B _deploy/$CW_SQFS_IMAGE:$CW_INSTALLATION_PATH:image-src=/ _deploy/$CW_CONTAINER_IMAGE"
